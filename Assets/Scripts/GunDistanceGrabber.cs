...
public class GunDistanceGrabber : OVRGrabber
{
    ...
    GunDistanceGrabber m_otherHand;

    protected GunDistanceGrabable m_target;
    ...
    protected override void Start()
    {
        ...
        GunDistanceGrabber[] grabbers = FindObjectsOfType<GunDistanceGrabber>();
    ...
    {
        GunDistanceGrabable closestGrabbable = m_target;
        ...
        if (closestGrabbable != null)
        {
            ...
            {
                ((GunDistanceGrabber)closestGrabbable.grabbedBy).OffhandGrabbed(closestGrabbable);
            }
    ...
    protected override void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
    {
        ...
        {
            m_grabbedObjectRotOff = m_grabbedObj.snapOffset.rotation * m_grabbedObjectRotOff;
            if (m_controller == OVRInput.Controller.LTouch) m_grabbedObjectRotOff =
                    Quaternion.Inverse(m_grabbedObjectRotOff);
        }
    ...
    static private GunDistanceGrabable HitInfoToGrabbable(RaycastHit hitInfo)
    {
        if (hitInfo.collider != null)
        {
            GameObject go = hitInfo.collider.gameObject;
            return go.GetComponent<GunDistanceGrabable>() ?? go.GetComponentInParent<GunDistanceGrabable>();
        }
        return null;
    }

    protected bool FindTarget(out GunDistanceGrabable dgOut, out Collider collOut)
    {
        dgOut = null;
        collOut = null;
        ...
        {
            GunDistanceGrabable grabbable = cg as GunDistanceGrabable;
    ...
    protected bool FindTargetWithSpherecast(out GunDistanceGrabable dgOut, out Collider collOut)
    {
        dgOut = null;
        collOut = null;
        ...
        {
            GunDistanceGrabable grabbable = null;
            Collider hitCollider = null;
            if (hitInfo.collider != null)
            {
                grabbable = hitInfo.collider.gameObject.GetComponentInParent<GunDistanceGrabable>();
            ...
            if (grabbable != null && m_preventGrabThroughWalls)
            {
                ...
                if (Physics.Raycast(ray, out obstructionHitInfo, m_maxGrabDistance, 1 << m_obstructionLayer, QueryTriggerInteraction.Ignore))
                {
                    GunDistanceGrabable obstruction = null;
                    if (hitInfo.collider != null)
                    {
                        obstruction = obstructionHitInfo.collider.gameObject.GetComponentInParent<GunDistanceGrabable>();
                    }
